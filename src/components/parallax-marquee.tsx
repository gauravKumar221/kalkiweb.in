...
export default function ParallaxMarquee() {
  ...
  useEffect(() => {
    ...
    // Clean up to prevent duplicate raf triggers
    return () => {
      lenis.destroy();
    };
  }, []);
  ...
  return (
    <main className="overflow-hidden bg-black py-20 relative selection:bg-primary selection:text-black">
...
const Slide: React.FC<SlideProps> = ({ src, direction, left, text, progress }) => {
  ...
  return (
    <motion.div style={{ x: translateX, left }} className="relative flex whitespace-nowrap gap-4">
...
const Phrase: React.FC<PhraseProps> = ({ src, text }) => {
  return (
    <div className="px-6 flex gap-6 items-center shrink-0">
