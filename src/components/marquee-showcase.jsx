"use client";
const MarqueeContent = ({ texts, }) => (<div className="flex">
    {texts.map(({ text, font }, index) => (<span key={index} className={`mx-8 ${font}`}>
        {text}
      </span>))}
  </div>);
export default function MarqueeShowcase() {
    ...
    return (<div className="relative w-full overflow-hidden flex flex-col justify-between !bg-white  py-16 md:py-24 border-t border-b border-border" style={{
      ...
      <div className="marquee w-full overflow-hidden whitespace-nowrap text-8xl md:text-[10rem] text-neutral-900 font-bold z-10 py-8">
        <div className="marquee-track inline-flex w-max animate-marquee-scroll">
          <div className="flex">
            {backMarqueeTexts.map((item, index) => (<span key={index} className={`mx-8 ${item.font}`}>
                {item.text}
              </span>))}
          </div>
          <div className="flex">
