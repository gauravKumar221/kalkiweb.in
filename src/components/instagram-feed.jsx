...
const InstagramPost = ({ post }) => (<div className="group relative aspect-square overflow-hidden">
        <Image src={post.src} data-ai-hint={post.hint} alt="Instagram Post" fill className="object-cover transition-transform duration-300 group-hover:scale-110"/>
...
export default function InstagramFeed() {
    return (<section className="py-12 border-y border-border/20">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {feedData.images.map((post) => (<InstagramPost key={post.src} post={post}/>))}
      </div>
    </section>);
}
