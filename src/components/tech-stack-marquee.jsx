const MarqueeRow = ({ items, duration }) => {
    const animationStyle = { '--t': duration };
    return (<div className="relative flex w-full max-w-4xl overflow-hidden [mask-image:linear-gradient(90deg,transparent,white_30%,white_70%,transparent)]">
            <div className="whitespace-nowrap animate-marquee" style={animationStyle}>
                {items.map((item, index) => (<span key={index} className="inline-flex m-2.5 px-2.5 py-1.5 rounded-md tracking-[0.3em] uppercase bg-[#333] text-white transition-colors duration-500 hover:bg-primary cursor-pointer">{item}</span>))}
            </div>
...
export default function TechStackMarquee() {
    ...
    return (<section className="py-16 md:py-24 bg-card text-card-foreground">
