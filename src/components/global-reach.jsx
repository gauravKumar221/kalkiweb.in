...
export default function GlobalReach() {
    ...
    return (<section ref={sectionRef} className="bg-white text-black py-12 overflow-hidden">
            <motion.div style={{ x }} className="flex items-center gap-8 whitespace-nowrap">
                <div className="bg-green-300 rounded-full flex items-center px-8 py-4 shrink-0">
                    ...
                    <motion.div style={{ x: arrowX }} className="ml-4 w-12 h-12 bg-black rounded-full flex items-center justify-center text-white">
                        <ArrowRight />
                    </motion.div>
                </div>
                
                <div className="flex items-center gap-8">
                    {[...flags, ...flags].map((flag, index) => (<motion.div key={index} style={{ rotate }} className="w-16 h-16 rounded-full overflow-hidden shadow-md shrink-0">
                            <Image src={flag.src} alt={flag.name} width={64} height={64} className="w-full h-full object-cover"/>
                        </motion.div>))}
                </div>
            </motion.div>
        </section>);
