...
const LetterCircle = ({ letter }) => (<motion.div drag dragConstraints={{ top: -150, left: -400, right: 400, bottom: 150 }} dragElastic={0.1} className={`w-24 h-24 md:w-32 md:h-32 rounded-full flex items-center justify-center text-white font-bold text-5xl md:text-6xl cursor-grab active:cursor-grabbing ${letter.color}`} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
    {letter.char}
  </motion.div>);
export default function DraggableLetters() {
    const constraintsRef = useRef(null);
    return (<section className="bg-[#DBCDAF] text-[#333] py-20 md:py-32 overflow-hidden">
      <div ref={constraintsRef} className="container mx-auto text-center relative">
        ...
        <div className="flex justify-center items-center gap-2 md:gap-4 mb-4">
            {letters.map((letter, index) => (<LetterCircle key={index} letter={letter}/>))}
        </div>
        <div className="flex justify-center items-center gap-2 md:gap-4">
            {webLetters.map((letter, index) => (<LetterCircle key={index} letter={letter}/>))}
        </div>
