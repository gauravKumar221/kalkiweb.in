...
const MarqueeRow = ({ testimonials, direction = 'left', speed = 50 }) => {
    ...
    return (<motion.div className="flex gap-4" variants={marqueeVariants} animate="animate">
      {extendedTestimonials.map((testimonial, index) => (<Card key={index} className="w-80 shrink-0 flex-none transition-all duration-300 hover:scale-105">
...
export default function TestimonialMarquee({ testimonials, speed = 50 }) {
    ...
    const rows = Array.from({ length: numRows }, (_, i) => {
        ...
        if (rowTestimonials.length > 0)
            return rowTestimonials;
        return null;
    }).filter(row => row !== null);
    return (<div className="relative flex flex-col gap-4 w-full overflow-hidden py-4">
      {rows.map((row, index) => (<MarqueeRow key={index} testimonials={row} direction={index % 2 === 0 ? 'left' : 'right'} speed={speed}/>))}
