...
export default function ProcessSection() {
    ...
    return (<section className="w-full py-16 md:py-24 bg-background">
            <div className="container mx-auto">
                <div className="md:grid md:grid-cols-12 md:gap-8 mb-12 items-end">
                    ...
                    <div className="md:col-span-8 flex flex-col md:flex-row justify-between items-end gap-6">
                        ...
                        <div className="flex gap-2 shrink-0">
                            <button onClick={() => scroll("left")} disabled={!canScrollLeft} className={`w-10 h-10 rounded-full flex items-center justify-center border border-primary text-primary hover:bg-primary hover:text-primary-foreground transition-all duration-300 ${!canScrollLeft ? "opacity-30 cursor-not-allowed" : "hover:scale-105"}`} aria-label="Scroll left">
                                <ArrowLeft className="w-5 h-5"/>
                            </button>
                            <button onClick={() => scroll("right")} disabled={!canScrollRight} className={`w-10 h-10 rounded-full flex items-center justify-center border border-primary text-primary hover:bg-primary hover:text-primary-foreground transition-all duration-300 ${!canScrollRight ? "opacity-30 cursor-not-allowed" : "hover:scale-105"}`} aria-label="Scroll right">
                ...
                <div ref={scrollContainerRef} className="flex md:flex-row overflow-x-auto overflow-y-hidden snap-x snap-mandatory scroll-smooth pb-8 md:space-x-8 space-y-8 md:space-y-0">
                    {processSteps.map((step, index) => (<div key={index} className="snap-start shrink-0 w-full md:w-1/3">
                            <Card className={cn("bg-card/50 border h-full flex flex-col group hover:bg-card/80 transition-colors duration-300", step.borderColor)}>
