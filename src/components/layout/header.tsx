...
import { ChevronDown, Phone, ArrowRight, MessageCircle, Mail, FileText, Linkedin, ShoppingBag, X } from "lucide-react";
...
function NavMenu({ isMobile = false, onClose }: { isMobile?: boolean, onClose?: () => void }) {
  ...
  if(isMobile) {
    return (
        <div className="container mx-auto pt-24 px-4 h-full text-black overflow-y-auto pb-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <div className="space-y-4">
                    <h3 className="text-sm text-gray-500 uppercase tracking-widest">Company</h3>
                ...
                <div className="space-y-4">
                    <h3 className="text-sm text-gray-500 uppercase tracking-widest">Services</h3>
                    <ul className="space-y-2">
                ...
                <div className="space-y-8">
                     <div>
                        <h3 className="text-sm text-gray-500 uppercase tracking-widest mb-2">Subscribe to our Newsletter</h3>
                         <form className="flex">
      ...
      <nav>
        <ul className="hidden md:flex items-center gap-6">
            {mainNavLinks.map(link => (
                <li key={link.href}>
                     <Link href={link.href} className={cn("transition-colors hover:text-primary text-sm font-medium", pathname === link.href ? "text-primary": "text-muted-foreground")}>
                           {link.label}
                    </Link>
                </li>
            ))}
...
export function Header() {
  ...
  return (
    <header className="fixed top-5 left-0 right-0 z-50 flex justify-center px-4">
      <div className="w-full max-w-7xl flex h-20 items-center justify-between text-foreground bg-background/50 border border-border backdrop-blur-md rounded-full px-4 md:px-8">
        <div className="flex-1 flex justify-start">
            <button
              ...
              >
                 <div className="flex flex-col justify-center items-center h-10 w-10 bg-muted rounded-full transition-colors group-hover:bg-primary">
                    <motion.div
                      ...
                      className="w-6 h-0.5 bg-foreground group-hover:bg-primary-foreground"
                    />
                    <motion.div
                       animate={open ? "open" : "closed"}
                       variants={{
                      ...
                      className="w-6 h-0.5 bg-foreground group-hover:bg-primary-foreground mt-1.5"
                    />
                 </div>
                 <span className={cn("font-medium text-sm tracking-wider uppercase hidden md:inline", open && "text-black")}>Menu</span>
              </button>
        </div>
       ...
       {open && (
           <motion.div 
                ...
                animate="open"
                exit="closed"
                className="fixed left-0 w-full h-screen bg-neutral-100 z-40 origin-top overflow-y-auto"
           >
               ...
               <div className="fixed top-5 right-5 z-50">
                    ...
                    >
                        <span className="font-medium text-sm tracking-wider uppercase text-black">Close</span>
