...
export default function ExpertiseShowcase() {
    return (<section className="py-16 md:py-32 bg-background">
            <div className="container">
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                        {expertiseItems.map((item, index) => (<Card key={index} className={cn('p-4 rounded-2xl transition-all duration-300 h-24', item.variant === 'dark' ? 'bg-foreground text-background' : 'bg-card text-foreground border border-border/20')}>
                               <div className="flex items-center justify-between h-full">
                                    ...
                                    {item.value && (<p className="font-bold text-lg text-primary">{item.value}</p>)}
                                </div>
                            </Card>))}
                    </div>
                    <div className="lg:col-span-1">
                        <Card className="p-4 h-full flex flex-col bg-card border border-border/20 rounded-2xl">
                            <div className="relative aspect-video w-full rounded-lg overflow-hidden group">
                                <Image src="https://picsum.photos/800/450" alt="Digital Transformation" data-ai-hint="company office" fill className="object-cover"/>
