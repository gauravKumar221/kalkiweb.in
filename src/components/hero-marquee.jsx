...
const MarqueeContent = ({ isLogo = false }) => {
  if (isLogo) {
    return (
      <>
        {logoTexts.map(({ text, font }, index) => (
          <Link
            ...
            key={index}
            className={cn(
              "text-2xl font-bold tracking-widest uppercase px-8 text-foreground whitespace-nowrap",
              font,
            )}
          >
...
const HeroMarquee = ({ isLogo = false }) => {
  return (
    <div
      className={cn(
        "group flex overflow-hidden p-4 gap-16",
        !isLogo && "bg-background/50  backdrop-blur-sm",
      )}
    >
      ...
      <div
        aria-hidden="true"
        className="flex min-w-full shrink-0 items-center justify-around gap-16 group-hover:[animation-play-state:paused] animate-marquee-horizontal"
      >
        <MarqueeContent isLogo={isLogo} />
        <MarqueeContent isLogo={isLogo} />
      </div>
